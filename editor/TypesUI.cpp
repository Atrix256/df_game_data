...
static void AddUIForType(const flatbuffers::Parser& parser, const flatbuffers::FieldDef& fieldDef, wxPanel* panel, wxSizer* sizer, json& json, int indent)
{
    ...
    label->Wrap(-1);
    labelSizer->Add(label, 0, wxEXPAND | wxALL, 5);
...
void AddUIForType(const flatbuffers::Parser& parser, const flatbuffers::StructDef& structDef, wxPanel* panel, wxSizer* sizer, json& json, int indent)
{
    ...
    for (const flatbuffers::FieldDef* fieldDef : structDef.fields.vec)
        AddUIForType(parser, *fieldDef, panel, sizer, json, indent+1);
}
